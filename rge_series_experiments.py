...
python rge_series_experiments.py --unit_test --max_iterations 3 --micro_batch_size 1 --seq_length 10 --device cuda:1
"""
...
def train_step(model, args, batch_np, loss_closure, optimizer=None):
    ...
    """
    if args.use_bptt:                   # ---------- first-order -----------
        if optimizer is None:
            raise ValueError("optimizer must be provided when --use_bptt")
...
def train(args):
    ...
    dtype = torch.float32 if args.use_bptt else torch.float16 # bptt in fp16 is unstable... try it but be weary.. vanishing/exploding grads await you 
    ...
    param_count  = sum(p.numel() for p in model_params)
    bytes_per_param = 4 if args.use_bptt else 2
    ...
    for iteration in range(args.max_iterations):
        ...
        def compute_loss(return_acc=False):
            ...
            logits, _, _ = model(xemb, require_gradients=args.use_bptt)                  # same for LSTM & DNC
    ...
    results['train_metrics']['iterations'].append(args.max_iterations+1)
    results['train_metrics']['time'].append(0)
...
# def build_args(run_cfg, seq_len, iters, device="cuda:0"):
#     """
#     Convert one entry from `all_runs` into an argparse-like Namespace that
#     contains *every* attribute expected by `train()` and by the project-wide
#     `argparse` spec, using sensible defaults where the run-specific config
#     does not supply a value.
#     """
#     args = argparse.Namespace()

#     # ------------------------------------------------------------------ #
#     #  Core – taken from run_cfg or overridden here                      #
#     # ------------------------------------------------------------------ #
#     args.device              = device          
#     args.task                = "penn_tree_bank"
#     args.tokenizer           = "char_level"
#     args.seq_length          = seq_len
#     args.max_iterations      = iters
#     args.log_interval        = max(1, iters // 2)      # at least 2 logs / run
#     args.micro_batch_size    = run_cfg["micro_batch_size"]
#     args.macro_batch_size    = run_cfg["macro_batch_size"]
#     args.hidden_size         = run_cfg["hidden_size"]
#     args.num_heads           = run_cfg["num_heads"]
#     args.head_size           = run_cfg["head_size"]
#     args.model_type          = run_cfg["model_type"]

#     # ------------------------------------------------------------------ #
#     #  Optimisation & algorithm choice                                   #
#     # ------------------------------------------------------------------ #
#     args.optimizer           = run_cfg["optimizer"]           # "bptt" | "cdrge" | "fdras"
#     args.use_bptt            = args.optimizer == "bptt"
#     args.learning_rate_and_epsilon = run_cfg["lr"]
#     args.use_adam            = False

#     # Finite-difference options
#     args.num_perturbations   = run_cfg.get("num_perturbations", 0)
#     args.antithetic          = run_cfg.get("antithetic", True)
#     args.antithetic_sampling = args.antithetic               # alias for parser flag
#     args.use_adaptive_step   = run_cfg.get("use_adaptive_step", False)

#     # Distribution / probes
#     args.distribution        = "rad"

#     # ------------------------------------------------------------------ #
#     #  Misc / training convenience                                       #
#     # ------------------------------------------------------------------ #
    
#     args.grad_clip           = 0.0
#     args.max_num             = 10             # only for the "add" arithmetic task
#     args.overfit_to_one_batch_flag = True     # unit tests: deterministic behaviour
...
#     # ------------------------------------------------------------------ #
#     #  Model architecture fields not used in the runs but required       #
#     # ------------------------------------------------------------------ #
#     args.input_size          = 100
#     args.memory_size         = 128

#     # The parser exposes `iterations`; mirror `max_iterations` for safety
#     args.iterations          = args.max_iterations

#     # ------------------------------------------------------------------ #
#     #  Experiment-tracking (disabled for unit tests)                      #
#     # ------------------------------------------------------------------ #
#     args.wandb               = False
#     args.no_wandb            = True
...
# def mem_estimate_gb(B, T, V, G, N, D, S, use_bptt):
#     """Roughly replicates `estimate_memory_usage()` logic inside `train()`."""
#     bytes_per_param = 4 if use_bptt else 2
#     param_cnt = (V * G * N * D) + (G * N * D * D) + (G * N * D) + (V * N * D) + V
#     param_mem = param_cnt * bytes_per_param
#     act_mem   = (
#         B * T * V * bytes_per_param +               # one-hot
#         B * T * G * N * D * bytes_per_param +       # embeddings
#         S * B * T * N * D * bytes_per_param +       # hidden states
#         B * T * V * bytes_per_param                 # logits
#     )
#     total_mb = (param_mem + act_mem) / (1024 ** 2)
...
def run_unittest(args):
    ...
    all_runs = {
            ...
            "tiny-bptt-default-dnc":   dict(model_size="tiny",   hidden_size=240,   num_heads=12,  head_size=20, memory_size=128, model_type="DNC",
                                        use_bptt=True, use_adam=False, learning_rate_and_epsilon=0.01,   micro_batch_size=int(batch_size/1), macro_batch_size=1,
            ...
            "medium-bptt-default-dnc": dict(model_size="medium", hidden_size=9600,  num_heads=64,  head_size=150, model_type="DNC",
                                        use_bptt=True, use_adam=False, learning_rate_and_epsilon=0.0001,  micro_batch_size=int(batch_size/1), macro_batch_size=1,
                                        description="Medium dnc with BPTT "),
            "medium-bptt-default-dnc": dict(model_size="medium", hidden_size=9600,  num_heads=64,  head_size=150, model_type="DNC",
                                            use_bptt=True, use_adam=True, learning_rate_and_epsilon=0.0001,  micro_batch_size=int(batch_size/1), macro_batch_size=1,
            ...
            "tiny-bptt-default-lstm":   dict(model_size="tiny",   hidden_size=240,   num_heads=12,  head_size=20, model_type="LSTM",
                                        use_bptt=True, use_adam=True, learning_rate_and_epsilon=0.01,   micro_batch_size=int(batch_size/1), macro_batch_size=1,
            ...
            #                             use_bptt=True, use_adam=True, learning_rate_and_epsilon=0.001,  micro_batch_size=int(batch_size/128), macro_batch_size=128,
...
def get_argument_parser():
    ...
    parser.add_argument("--use_bptt", action="store_true", default=False,
